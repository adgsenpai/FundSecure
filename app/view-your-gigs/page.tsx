...
const SponsorShowcasePage = () => {
    ...
    const fetchProjects = async (page = 1, search = '', username = '') => {
        setLoading(true); // Set loading to true before fetching data
        try {
            const response = await fetch(`/api/sponsor-showcase?page=${page}&search=${search}&username=${username}`);
    ...
    useEffect(() => {
        fetchProjects(currentPage, searchTerm, username);
